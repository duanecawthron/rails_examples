...
cat << EOF > app/views/posts/index.js.erb
<% sleep 2 %>
...
#
# https://developer.mozilla.org/En/Window.setTimeout (once)
# https://developer.mozilla.org/En/Window.setInterval (repeats)
...
function checkScroll() {
  if (nearBottomOfPage()) {
    currentPage++;
	\$.ajax({
		url: "http://localhost:3000/posts.js?page=" + currentPage,
		dataType: "script",
		type: "GET"
	});
  } else {
